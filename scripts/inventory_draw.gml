...
for (i=0; i<slot_height; i+=1)
{
    for (j=0; j<slot_width; j+=1)
    {
        ...
        if index = global.ItemSelected{
           if point_in_rectangle(mouse_x,mouse_y,_x+((spr_width+slot_gap)*j),_y+((spr_height+slot_gap)*i),_x+((spr_width+slot_gap)*j)+spr_width,_y+((spr_height+slot_gap)*i)+spr_height){
        ...
        if ds_map_find_value(global.inventory,"slot"+string(index)) = true{
           ...
           if global.MouseItem = false{
              ...
              if global.MouseIndex != index{
                 ...
                 draw_sprite(sprite,image_index,mouse_x-(item_width/2),mouse_y-(item_height/2)) 
              }
           }
        }
        index += 1
...
for (i=0; i<slot_height; i+=1)
{
    ...
    if ds_map_find_value(global.inventory,"slot"+string(index)) = true{
      if global.MouseItem = false{
      if point_in_rectangle(mouse_x,mouse_y,_x+((spr_width+slot_gap)*j),_y+((spr_height+slot_gap)*i),_x+((spr_width+slot_gap)*j)+spr_width,_y+((spr_height+slot_gap)*i)+spr_height){
         draw_panel(sprite_infobox,0,max(view_xview[0],mouse_x-string_width(string(item_id_read(inventory_read("ID",index),0)))/2),floor(_y+((spr_height+slot_gap)*i)-5-string_height(string(item_id_read(inventory_read("ID",index),0)))/2-15),string_width(string(item_id_read(inventory_read("ID",index),0)))+10,string_height(string(item_id_read(inventory_read("ID",index),0)))+10,10)
         draw_text(max(view_xview[0]+5,mouse_x+5+-string_width(string(item_id_read(inventory_read("ID",index),0)))/2),floor(_y+((spr_height+slot_gap)*i)-string_height(string(item_id_read(inventory_read("ID",index),0)))/2-15),string(item_id_read(inventory_read("ID",index),0)))
      }
      }
    }
    index += 1
